function [dataset]=ar_dataset(model,nulldata,y,u,t,timeoffset)
...
t=t+timeoffset; %offset temporale
...
elseif n==0
    for index=1:model.numcat
        indexes=find((z>=cat_intervals(index,1)).*(z<cat_intervals(index,2)));
        dataset.catDirty(indexes,1)=index;
    end
else
    for index=1:model.numcat
        indexes=find((An(:,1)>=cat_intervals(index,1)).*(An(:,1)<cat_intervals(index,2)));
        dataset.catDirty(indexes,1)=index;
    end
end
